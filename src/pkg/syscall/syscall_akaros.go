...
func SyscallWrapper(trap, a1, a2, a3 uintptr) (r1, r2 uintptr, err error) {
	return Syscall6Wrapper(trap, a1, a2, a3, 0, 0, 0)
}
func Syscall6Wrapper(trap, a1, a2, a3, a4, a5, a6 uintptr) (r1, r2 uintptr, err error) {
	...
	// This is basically a plan9 hack. On Akaros, errno is *currently* only set
	// to EUNSPECIFIED if and only if we are making a file system related
	// syscall on a plan9 device.  The Go semantics of these syscalls on Plan9
	// say that if and only if __r1 == -1, then the error string passed back is
	// valid.  This means that if __r1 != -1, then we need to basically ignore
	// the error and pass back an Errno of 0.  Otherwise we just return the
	// EUNSPECIFIED errno.
	if (Errno(__err) == EUNSPECIFIED && __r1 != -1) {
		akaerror = nil
	}
