...
func dialPlan9(net string, laddr, raddr Addr) (fd *netFD, err error) {
	...
	data, err := os.OpenFile(os.Nsprefix+"/net/"+proto+"/"+name+"/data", os.O_RDWR, 0)
	if err != nil {
		f.Close()
		return nil, &OpError{"dial", net, raddr, err}
	}
	laddr, err = readPlan9Addr(proto, os.Nsprefix+"/net/"+proto+"/"+name+"/local")
	if err != nil {
		data.Close()
		f.Close()
...
func (l *netFD) acceptPlan9() (fd *netFD, err error) {
	...
	raddr, err := readPlan9Addr(l.proto, os.Nsprefix+"/net/"+l.proto+"/"+name+"/remote")
	if err != nil {
		data.Close()
		f.Close()
		return nil, &OpError{"accept", l.proto, l.laddr, err}
