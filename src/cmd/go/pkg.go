...
func (p *Package) load(stk *importStack, bp *build.Package, err error) *Package {
	...
	// No longer allowed except in runtime, runtime/cgo, or runtime/parlib, for now.
	if len(p.CFiles) > 0 && !p.usesCgo() && (!p.Standard || (p.ImportPath != "runtime" && p.ImportPath != "runtime/parlib")) {
		p.Error = &PackageError{
			ImportStack: stk.copy(),
